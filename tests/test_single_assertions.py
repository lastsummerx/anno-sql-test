...
def test_assert_predicate_pass():
    ...
    result = evaluator.evaluate(SingleAssertAll(predicate=ExprColumn(expr="a > 0")), [df])
    assert result.passed is True


def test_assert_predicate_fail():
    df = spark.createDataFrame([(1,), (None,)], ["a"])
    result = evaluator.evaluate(SingleAssertAll(predicate=ExprColumn(expr="a is not null")), [df])
...
def test_assert_any_pass():
    df = spark.createDataFrame([(1,), (2,), (3,)], ["a"])
    result = evaluator.evaluate(SingleAssertAny(predicate=ExprColumn(expr="a > 2")), [df])
    assert result.passed is True


def test_assert_any_fail():
    df = spark.createDataFrame([(1,), (2,), (3,)], ["a"])
    result = evaluator.evaluate(SingleAssertAny(predicate=ExprColumn(expr="a > 10")), [df])
    assert result.passed is False


def test_assert_any_empty_df_fail():
    df = spark.createDataFrame([], schema="a: int")
    result = evaluator.evaluate(SingleAssertAny(predicate=ExprColumn(expr="a > 0")), [df])
    assert result.passed is False
...
def test_assert_none_pass():
    df = spark.createDataFrame([(1,), (2,), (3,)], ["a"])
    result = evaluator.evaluate(SingleAssertNone(predicate=ExprColumn(expr="a > 10")), [df])
    assert result.passed is True


def test_assert_none_fail():
    df = spark.createDataFrame([(1,), (2,), (3,)], ["a"])
    result = evaluator.evaluate(SingleAssertNone(predicate=ExprColumn(expr="a > 2")), [df])
    assert result.passed is False


def test_assert_none_empty_df_pass():
    df = spark.createDataFrame([], schema="a: int")
    result = evaluator.evaluate(SingleAssertNone(predicate=ExprColumn(expr="a > 0")), [df])
    assert result.passed is True
...
def test_assert_all_failure_sample():
    ...
    result = sample_eval.evaluate(SingleAssertAll(predicate=ExprColumn(expr="a is not null")), [df])
    assert result.passed is False
...
def test_assert_any_failure_sample():
    sample_eval = SparkAssertionEvaluator(sample_count=3)
    df = spark.createDataFrame([(1,), (2,), (3,)], ["a"])
    result = sample_eval.evaluate(SingleAssertAny(predicate=ExprColumn(expr="a > 10")), [df])
    assert result.passed is False
    assert result.failure_sample is not None
    assert len(result.failure_sample) == 3
...
def test_assert_none_failure_sample():
    ...
    df = spark.createDataFrame([(1,), (2,), (3,)], ["a"])
    result = sample_eval.evaluate(SingleAssertNone(predicate=ExprColumn(expr="a > 2")), [df])
    assert result.passed is False
    assert result.failure_sample is not None
    assert len(result.failure_sample) == 1
